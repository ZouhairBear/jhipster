...
import tn.camelstudio.jhipster.service.BankAccountService;
import tn.camelstudio.jhipster.web.rest.errors.BadRequestAlertException;

/**
 * REST controller for managing {@link tn.camelstudio.jhipster.domain.BankAccount}.
 */
@RestController
@RequestMapping("/api")
public class BankAccountResource {
    ...
    public List<BankAccount> getAllBankAccounts(@RequestParam(required = false) String filter) {
        if ("userextra-is-null".equals(filter)) {
            log.debug("REST request to get all BankAccounts where userExtra is null");
            return bankAccountService.findAllWhereUserExtraIsNull();
        }
